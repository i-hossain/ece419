...
public class MazeImpl extends Maze implements Serializable, ClientListener, Runnable {
        ...
        // Map<String, Integer> deathMap = new ConcurrentHashMap<String, Integer>();
        ...
        public synchronized boolean clientFire(Client client) {
                ...
                myProjMap.put(prj.getOwner(), prj);
...
//                                Iterator it = myClientMap.keySet().iterator();
        ...
        public void runMoveProjectile(Client cl) {
                ...
                synchronized(projectileMap) {
                	synchronized (myProjMap) {
                        ...
                        deadPrj.addAll(moveProjectile(cl));              
                        Iterator it = deadPrj.iterator();
                        while(it.hasNext()) {
                                Object o = it.next();
                                assert(o instanceof Projectile);
                                Projectile prjd = (Projectile)o;
                                myProjMap.remove(prjd.getOwner());
                                projectileMap.remove(prjd);
                                clientFired.remove(prjd.getOwner());
                        }
                        deadPrj.clear();
                	}
                }
        ...
        private synchronized Collection moveProjectile(Client cl) {
        		Projectile prj = myProjMap.get(cl);
                ...
                /* Check if prj is destroyed already */
                if(cell.getContents() == null) {
                        // projectile is already dead.
                        deadPrj.add(prj);
                        return deadPrj;
                }
